...
/* Pieces below are specific to this workload's physical story (weights +
   KV cache living in plain system RAM instead of HBM, one append per
   generated token) and stay local to this file rather than living in the
   shared engine. Deliberately close to chatbot's Scene — same shape of
   story — with GPU/HBM swapped for CPU/DDR throughout. */
  ...
  return (
    <g>
      ...
      <text x={x + w / 2} y={y + 24} textAnchor="middle" fill={C.ink} fontSize={15} fontWeight={600}>
        System Memory <tspan fill={C.mut} fontSize={11} fontFamily={C.mono}>· DDR5</tspan>
      </text>
      ...
      <text x={innerX + innerW / 2} y={wTop + 28} textAnchor="middle" fill="#6c88a6" fontSize={10} fontFamily={C.mono}>the model (quantized)</text>
...
export default function CpuInferScene({ state, disp }) {
  const bn = PHASES[state.phase].bottleneck;
  const beatT = beatProgress(state, state.speed);

  return (
    <>
      {/* intake: just a network hop into the CPU — no separate host to
          cross, the CPU *is* the host. */}
      <SmallNode x={44} y={40} w={140} h={56} label="Network" util={disp.nic} />
      <FeedLine from={[184, 68]} to={[250, 158]} util={disp.nic} clock={state.clock} label="request in" />

      {/* the bandwidth channel (HERO): memory → compute, right to left.
          Same primitive as chatbot's HBM channel — the point is that it
          looks identical in *shape* and still runs far cooler in *scale*. */}
      <Channel x0={690} x1={350} top={206} bot={294} util={disp.memBw} clock={state.clock}
        bottleneck={bn === 'memBw'} label="SYSTEM MEMORY BANDWIDTH" directionHint="◄ weights + KV read into compute" />

      {/* append arc: compute → KV cache, one pulse per token */}
      <AppendArc beatT={beatT} />

      {/* CPU Matrix Engine (AMX) */}
      <BigNode x={150} y={158} w={200} h={168}
        title="CPU Matrix Engine" sub="AMX / AVX-512 tiles"
        util={disp.compute} bottleneck={bn === 'compute'}
        idleNote={state.phase === 'decode' ? 'mostly waiting' : 'well under GPU tensor-core scale'} />

      {/* System memory (weights + KV cache, in plain DDR5) */}
...
/* Terminal panel content — plain HTML/text (not SVG), rendered by App.jsx
   inside the shared engine/TerminalWindow chrome. Same reveal mechanics as
   chatbot's Terminal, different canned exchange to match the "no GPU on
   this box" framing. */
